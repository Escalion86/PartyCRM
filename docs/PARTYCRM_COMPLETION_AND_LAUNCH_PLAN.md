...
До полноценного анонса еще рано. Самые важные пробелы: транзакции в заказах пока не доведены до UI/API-сценария, документы используют artist-терминологию и не закрывают company-сценарий, push только сохраняет настройку, входящие лиды/Tilda отсутствуют, VK/Avito/Novofon настройки не дают полного PartyCRM lead-flow, Google Calendar не портирован, тарифы/биллинг нужно привязать к company-модели, а публичный лендинг обещает часть функций, которые еще не готовы.
...
- [ ] На лендинге временно убрать или смягчить обещания функций, которые еще не закрыты: Google Calendar, автоформирование документов, полноценные финансы, телефония, интеграции.
...
- [ ] Поддержать типы `income` и `expense`, категории `deposit`, `final_payment`, `client_payment`, `payout`, `refund`, `taxes`, `materials`, `travel`, `other`.
- [x] В модалке заказа отдельно показывать: договорная сумма, получено, остаток, расходы и валовая маржа по новым транзакциям.
- [ ] Добавить статусы выплат исполнителям: запланировано, готово к выплате, выплачено, отменено.
- [ ] При закрытии заказа проверять: получены деньги, выплаты исполнителям отмечены, открытые задачи обработаны.
- [ ] Дать владельцу/админу фильтры: ждет предоплату, есть долг, есть невыплаченные исполнители, отрицательная маржа.
- [ ] Добавить автосохранение нового заказа перед первой транзакцией: текущий инкремент показывает транзакции после сохранения заказа.
- [ ] Перевести summary списков/дашборда с legacy `order.transactions/clientPayment` на отдельную коллекцию `transactions`.
- [ ] Сделать экспорт финансов за период в CSV.
- [ ] Покрыть unit-тестами расчет маржи и статусов оплаты.

### 4. Усилить роли и доступы

- [ ] Завершить документацию прав в `docs/PARTYCRM_ROLES.md`: кто видит клиентские суммы, выплаты, контакты, настройки, тарифы, интеграции.
- [ ] Проверить route guard на прямые URL `/company/settings/integrations`, `/company/settings/documents`, `/company/settings/notifications`, `/company/settings/tariffs`.
- [ ] Разделить глобальную роль PartyUser (`admin/dev`) и роль в компании (`owner/admin/performer`) во всех settings/API flow.
- [ ] Добавить тесты для `companySettingsTabs`, `getPartyRequestContext`, API management-only endpoints.
- [ ] Запретить исполнителю доступ к `/company/*`, если режим интерфейса только performer.
- [ ] Запретить показ полной клиентской суммы и внутренних заметок исполнителю во всех endpoints, не только в UI.

### 5. Сделать company settings полноценными

- [ ] `Общие`: добавить профиль компании, контакты, юридическое название, город по умолчанию, формат нумерации заказов.
- [ ] `Списки`: добавить типы услуг/мероприятий, статусы подготовки, источники заявок, способы оплаты, категории расходов.
- [ ] `Документы`: переименовать `artist*` поля в company/provider terms, сохранить backward-compatible migration.
- [ ] `Интеграции`: добавить кнопки `Проверить`, `Подключить`, `Отключить`, `Скопировать webhook`, последняя ошибка, последнее событие.
- [ ] `Уведомления`: связать флаг push с реальными подписками и доставкой.
- [ ] `Тарифы`: показывать текущий тариф компании, лимиты, окончание периода, историю платежей и CTA оплаты.
...
- [ ] Создать `/api/party/public/lead` с API-key авторизацией на уровне компании.
- [ ] Создать `/api/party/public/lead/tilda` с нормализацией payload Tilda.
- [ ] Добавить несколько именованных API-ключей источников в settings компании.
- [ ] Создавать или обновлять `PartyClient` по телефону/контактам.
- [ ] Создавать `PartyOrder` в статусе `draft` с источником, точкой, услугой, датой/временем и комментарием.
- [ ] Добавить маршрутизацию по точке/услуге/источнику.
- [ ] Добавить push/in-app уведомление администраторам компании о новой заявке.
- [ ] Подготовить пользовательскую инструкцию по API и Tilda.

### 3. Push и напоминания

- [ ] Создать Party push subscription model или расширить существующую модель с product/company context.
- [ ] Добавить `/api/party/push/public-key`, `/subscribe`, `/unsubscribe`, `/test`.
- [ ] Отправлять push по новым Party leads.
...
- [ ] Решить модель календаря: company calendar, календарь владельца, календарь администратора или персональные календари исполнителей.
- [ ] Подключить OAuth flow на уровне компании.
- [ ] Синхронизировать PartyOrder и `additionalEvents`.
- [ ] В событии календаря показывать точку/адрес, услуги, исполнителей, статус оплаты.
- [ ] Учитывать изменения даты/исполнителя/точки и удаление/отмену заказа.
- [ ] Не показывать исполнителю клиентскую сумму в его calendar export, если появится персональный календарь исполнителя.
...
- [ ] Добавить trial period и onboarding после оплаты/активации.
- [ ] Привязать feature flags к тарифам: лимит сотрудников, лимит заказов, документы, интеграции, Google Calendar, телефония, AI, аналитика.
