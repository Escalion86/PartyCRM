...
- [-] Реализовать отдельный OAuth и ограниченный calendar export для кабинета исполнителя без клиентской суммы: безопасный ICS export `/api/party/performer/calendar` готов, отдельный OAuth остаётся будущим расширением.
