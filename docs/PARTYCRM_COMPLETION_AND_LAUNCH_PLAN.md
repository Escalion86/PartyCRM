...
| Google Calendar    | Подключение, import/export/sync, reminders                     | Company OAuth и односторонний export заказов готовы; performer OAuth/export синхронизирует только подтверждённые назначения без финансов компании | Проверить реальные Google OAuth сценарии на pilot-данных                                    |
...
- [ ] На лендинге временно убрать или смягчить обещания функций, которые еще не закрыты: телефония и входящие интеграции.
...
- [x] Реализовать отдельный OAuth и ограниченный calendar export для кабинета исполнителя без клиентской суммы: безопасный ICS export `/api/party/performer/calendar` сохранён, Google Calendar синхронизирует только подтверждённые/выполненные назначения.
...
- [x] Для AI сделать company-level keys/providers и подтверждаемый черновик заказа: подтверждаемый draft готов в Novofon flow; company-level save/check routes проверяют `allowAi`, а UI показывает key/provider поля и диагностику последней проверки.
...
- [x] Зафиксировать balance-first UX/API: provider endpoints YooKassa/Tochka принимают только пополнение баланса компании, successful sync/webhook только пополняет баланс, а тариф выбирается через `/api/party/billing/tariff/select` и списывается с баланса.
- [x] Добавить безопасный billing preflight diagnostics: `/api/party/billing/diagnostics` и блок во вкладке тарифов показывают готовность YooKassa/Tochka к тестовому платежу, missing env, webhook URL и блокеры без раскрытия секретов.
...
- [x] Owner создает точку, услугу, клиента, сотрудника без аккаунта и заказ.
- [x] Система показывает конфликт, если точка или исполнитель заняты.
- [x] Заказ содержит сумму клиента, предоплату, остаток, расходы, выплаты и маржу.
- [x] Исполнитель видит назначение в `/performer`, подтверждает участие и отмечает выполнение.
- [x] Owner закрывает прошедший заказ, видит финансовый результат и статус выплат.
- [x] Входящая заявка из API/Tilda создает PartyOrder `draft` и уведомляет администратора.
- [x] Договор и акт по заказу скачиваются в DOCX с реквизитами компании и клиента.
- [x] Настройки интеграций показывают реальные статусы и последнюю ошибку: VK, Avito, Novofon и AI имеют company-level check routes и сохраняют `lastCheckedAt`/`lastError` в `PartyCompany.settings.integrations`.
- [ ] Тариф/оплата работают на тестовом платеже и корректно меняют доступные функции.
- [x] Исполнитель не видит клиентскую сумму и внутренние заметки ни через UI, ни через API.
- [x] Чужой `x-partycrm-company-id` возвращает `403 partycrm_company_access_denied`.
