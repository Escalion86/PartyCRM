...
- Для вкладок `Интеграции`, `Уведомления`, `Документы`, `Тарифы` используй доступ только для глобальных ролей `admin` и `dev`.
...
- Пункты меню, доступные только `admin/dev`, помечай жёлтой звёздочкой.
