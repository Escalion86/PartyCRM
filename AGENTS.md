...
- Файл: `docs/PARTYCRM_ROADMAP.md`.
- В этом проекте roadmap называется именно `PARTYCRM_ROADMAP.md`; файла `docs/ROADMAP.md` нет.
...
- 1) Прочитай `docs/PARTYCRM_ROADMAP.md` (приоритеты, активные эпики, что уже сделано).
...
## Правила доступа PartyCRM
- Если задача затрагивает `app/company/settings`, различай глобальную роль пользователя `PartyUser.role` и роль участия в компании `membership.role`.
- Все вкладки `Настройки компании` доступны только membership-ролям `owner` и `admin`; глобальная роль `PartyUser.role` сама по себе доступ не даёт.
- Вкладки `Интеграции`, `Уведомления` и `Тарифы` доступны при любом тарифе компании.
- Вкладка `Документы` остаётся видимой при любом тарифе, но функциональность внутри неё проверяет тарифный флаг `allowDocuments`.
- Для menu-only ограничений недостаточно скрыть пункт в UI: прямой переход по URL тоже должен проверяться route guard-логикой.
- Пункты меню, доступные только `dev`, помечай фиолетовой звёздочкой.
...
- Если изменение относится к roadmap/backlog, синхронизируй `docs/PARTYCRM_ROADMAP.md` в этом же PR/коммите.
...
- Если в рамках задачи закрыт любой пункт roadmap (`[x]`) в `docs/PARTYCRM_ROADMAP.md`, обязательно обнови версию приложения в `package.json` в этом же наборе изменений.
