...
import { reportFailure, validateReportAudience } from '@server/partyReportCore'
...
export const POST = withReportContext(async (req, context) => {
  ...
  let version = 1
  let audience = validateReportAudience(body.audience)
  if (body.previousTemplateId) {
    ...
    audience = validateReportAudience(body.audience, previous)
    familyId = previous.familyId
    version = previous.version + 1
  }
  const template = await Templates.create({ tenantId: context.tenantId, familyId, version, title, audience, stage: body.stage, active: body.active !== false, fields })
