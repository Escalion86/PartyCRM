...
export default function CompanySettingsListsContent({ activeCompanyId }) {
  ...
  return (
    <div className="grid gap-4">
      ...
      <DictionarySection
        key={`serviceTypes-${JSON.stringify(settings?.serviceTypes ?? [])}`}
        title="Типы услуг"
        subtitle="Используется для группировки услуг и будущей маршрутизации заявок"
        emptyLabel="Типов услуг пока нет"
        promptLabel="Новый тип услуги"
        field="serviceTypes"
      ...
      <DictionarySection
        key={`preparationStatuses-${JSON.stringify(settings?.preparationStatuses ?? [])}`}
        title="Статусы подготовки"
        subtitle="Будущие чек-листы и задачи заказа"
        emptyLabel="Статусов пока нет"
        promptLabel="Новый статус подготовки"
        field="preparationStatuses"
        items={settings?.preparationStatuses ?? []}
        saving={saving}
        savePatch={savePatch}
      />
      ...
      <DictionarySection
        ...
        subtitle="Для лидов, Tilda, VK, Avito и ручных заказов"
      ...
      <DictionarySection
        key={`paymentMethods-${JSON.stringify(settings?.paymentMethods ?? [])}`}
        title="Способы оплаты"
        subtitle="Для финансов и документов"
        emptyLabel="Способов оплаты пока нет"
        promptLabel="Новый способ оплаты"
      ...
      <DictionarySection
        key={`expenseCategories-${JSON.stringify(settings?.expenseCategories ?? [])}`}
        title="Категории расходов"
        subtitle="Для учета маржи и финансового экспорта"
        emptyLabel="Категорий расходов пока нет"
        promptLabel="Новая категория расходов"
