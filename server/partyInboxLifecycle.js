...
const getScheduleSettings = async (tenantId) => {
  const Company = await getPartyCompanyModel()
  const company = await Company.findOne({ _id: tenantId }).select('settings.timeZone settings.inboxSchedule').lean()
...
export const registerPartyInboxIncoming = async ({ tenantId, channel, sourceId, token, at, text = '', attachments = [], call = false, dependencies = {} }) => {
  ...
  const settings = dependencies.timeZone ? dependencies : await getScheduleSettings(tenantId)
  const receivedAt = new Date(at || Date.now())
  const dueAt = calculatePartyInboxResponseDueAt(receivedAt, settings.timeZone || 'Asia/Krasnoyarsk', settings.inboxSchedule)
